...
class CheckpointCallBack(BaseCallBack):
    ...
    def _save(self, trainer:ITrainer, file_name: str) -> None:
    ...
    def on_step_end(self, trainer: ITrainer, metrics: dict[str, float], step: int) -> None:
        if step > 0 and step % self.save_every_steps == 0:
            self._save(trainer, self.LATEST_CHECKPOINT_NAME)
    ...
    def on_epoch_end(self, trainer: ITrainer, epoch: int) -> None:
        ...
        if epoch > 0 and epoch % self.keep_every_epochs == 0:
            self._save(trainer, f"epoch_{epoch}.pt")
    
    def on_training_end(self, trainer: ITrainer) -> None:
        self._save(trainer, self.FINAL_CHECKPOINT_NAME)
